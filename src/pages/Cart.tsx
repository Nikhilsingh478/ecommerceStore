...
const Cart = () => {
  ...
  return (
    <div className="flex min-h-screen flex-col bg-background animate-fade-in pb-[calc(5rem+env(safe-area-inset-bottom))] md:pb-0">
      ...
      {/* Mobile header */}
      <div className="sticky top-0 z-40 flex md:hidden items-center gap-3 bg-white dark:bg-[#0a0a0a] border-b border-[#e2e8f0] dark:border-[#1f1f1f] px-4 py-3.5">
        <button
          onClick={() => navigate(-1)}
      ...
      <main className="flex-1 w-full mx-auto max-w-2xl px-0 md:px-6 flex flex-col pt-0 md:pt-10 md:pb-16">
        ...
        {items.length === 0 ? (
          ...
          <div className="flex flex-1 flex-col">
                ...
                >
                  ...
                  <div className="flex flex-1 flex-col gap-1 min-w-0">
                    ...
                    <div className="flex items-baseline gap-1.5">
                      <span className="text-[13px] font-semibold text-foreground">{formatPrice(item.product.offerPrice)}</span>
                    ...
                    <div className="mt-1.5 flex items-center gap-2">
                      <div className="flex items-center gap-1 rounded-xl border border-border bg-background p-0.5">
                        ...
                        <button
                          ...
                          className="flex h-6 w-6 items-center justify-center rounded-lg bg-foreground text-background active:scale-90 transition-transform"
                        >
                      ...
                      <button
                        ...
                        className="flex h-8 w-8 items-center justify-center rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20 active:scale-90 transition-all"
                      >
            ...
            {/* Savings */}
            {savings > 0 && (
              <div className="mx-4 md:mx-0 mt-2 flex items-center gap-2 rounded-xl bg-green-50 dark:bg-green-900/20 border border-green-100 dark:border-green-900/40 px-4 py-2.5 animate-fade-up">
            ...
            {deliveryFee > 0 && (
              <div className="mx-4 md:mx-0 mt-2 rounded-xl border border-border bg-card px-4 py-2.5">
                <span className="text-[12px] text-muted-foreground">
                  Add items worth {formatPrice(499 - totalPrice())} more for <span className="font-semibold text-foreground">FREE delivery</span>
                </span>
              </div>
            )}

            {/* Summary + Checkout */}
            <div className="sticky bottom-[calc(4.5rem+env(safe-area-inset-bottom))] md:relative md:bottom-auto z-40 mx-4 md:mx-0 mt-4 rounded-2xl bg-card border border-border overflow-hidden">
              <div className="flex items-center justify-between px-5 py-4">
                ...
                <button
                  ...
                  className="flex items-center justify-center min-w-[140px] rounded-xl bg-foreground px-8 py-3.5 text-[14px] font-medium text-background hover:opacity-90 active:scale-[0.97] transition-all"
                >
                  Checkout
                </button>
              </div>
